...
show_help() {
    echo "${0##*/} -i INFILE [-s FILESIZE] [-f TIMESTAMP] [-t TIMESTAMP] [--subs] [--amix] [--noaudio] [--crop <\"16:9\"|\"21:9\"|CROPSTRING>] [--downscale] [--hls] [--seekaccurate] [--filter \"filterstring\"] [--params \"parameters\"] [OUTFILE]"
    echo
...
OPTIONS=s:f:t:o:i:h
LONGOPTS=size:,from:,to:,output:,input:,help,subs,amix,noaudio,boostaudio,crop:,hls,seekaccurate,filter:,params:,downscale
...
if [[ -n "$phls" ]] || [[ -n "$psize" ]]; then
    ...
    if [[ -n "$phls" ]]; then
        ...
        output_flags=("-maxrate:v" "${target_video_bitrate_kbit_s}k")
        output_flags+=("-minrate:v" "2M")
        output_flags+=("-bufsize:v" "6M")
        output_flags+=("-g" "$min_iframe")
        output_flags+=("-sc_threshold" "0")
        output_flags+=("-keyint_min" "$min_iframe")
        #output_flags+=("-force_key_frames" "expr:gte(t,n_forced*2)") # might cause problems
        ...
        output_flags=("-f" "mp4")
        output_flags+=("-movflags")
        output_flags+=("+faststart")
    fi
