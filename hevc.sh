...
globalopts=()
x265opts=("-c:v" "libx265" "-preset" "medium" "-pix_fmt" "yuv420p" "-profile:v" "main444-8")
cudaopts=("-c:v" "hevc_nvenc" "-profile:v" "main" "-pixel_format" "yuv420p" "-preset:v" "p7" "-tune:v" "hq")
...
if [[ -n "$phls" ]] || [[ -n "$psize" ]]; then
    ...
    if [[ -n "$pcuda" ]]; then
        ...
        rm x265_2pass.log > /dev/null
        rm x265_2pass.log.cutree > /dev/null
    fi
